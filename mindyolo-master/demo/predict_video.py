...
def get_parser_infer(parents=None):
    ...
    parser.add_argument("--task", type=str, default="detect", choices=["detect", "segment", "video_detect", "video_segment"])
    ...
    parser.add_argument("--image_path", type=str, help="path to image (for image tasks)")
    parser.add_argument("--video_path", type=str, help="path to video (for video tasks)")
    parser.add_argument("--camera_idx", type=int, default=0, help="camera index (0/1/2, for video tasks)")
    parser.add_argument("--save_result", type=ast.literal_eval, default=True, help="whether save the inference result")
    parser.add_argument("--video_fps", type=int, default=30, help="FPS of output video")
    parser.add_argument("--show_window", type=ast.literal_eval, default=False, help="whether show result window (need GUI)")
...
def segment(
    ...
    for k, v in result_dict.items():
        if k == "segmentation":
            logger.info(f"{k} shape: {v[0].shape if v else 'empty'}")
        else:
            logger.info(f"{k}: {v}")
    logger.info(f"Speed: %.1f/%.1f/%.1f ms inference/NMS/total per %gx%g image at batch-size %g;" % t)
    logger.info(f"Detect a image success.")
...
def infer(args):
    ...
    # 视频处理逻辑
    if args.task in ["video_detect", "video_segment"]:
        # 打开视频源
        if isinstance(args.video_path, str) and os.path.isfile(args.video_path):
            cap = cv2.VideoCapture(args.video_path)
            print(f"✅ Using local video file: {args.video_path}")
        else:
            cap = cv2.VideoCapture(args.camera_idx)
            print(f"✅ Using camera (index {args.camera_idx}). If no image, try other indices.")
    
        if not cap.isOpened():
            raise ValueError(f"Failed to open video source (path: {args.video_path}, camera index: {args.camera_idx})")
        ...
        total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT)) if args.video_path else -1
        logger.info(f"Processing video: {args.video_path or 'camera stream'}, {frame_width}x{frame_height}, {total_frames} frames")

        # 准备视频输出和帧保存目录
        video_writer = None
        frame_save_dir = os.path.join(args.save_dir, "temp_frames")  # 临时保存绘制后的帧
        os.makedirs(frame_save_dir, exist_ok=True)
        if args.save_result:
            save_path = os.path.join(args.save_dir, "video_results")
            os.makedirs(save_path, exist_ok=True)
            output_video_path = os.path.join(save_path, "output.mp4")
            fourcc = cv2.VideoWriter_fourcc(*'mp4v')
            video_writer = cv2.VideoWriter(output_video_path, fourcc, args.video_fps, (frame_width, frame_height))
        ...
        try:
            while cap.isOpened():
                ...
                frame_count += 1
                logger.info(f"Processing frame {frame_count}/{total_frames if total_frames != -1 else 'unknown'}")

                # 1. 对当前帧执行检测/分割
                if args.task == "video_detect":
                    result_dict = detect(
                        network=network,
                        img=frame,
                ...
                # 2. 调用 draw_result 绘制结果（关键修复）
                # 2.1 先保存原始帧为临时文件（供 draw_result 读取）
                temp_original_path = os.path.join(frame_save_dir, f"original_frame_{frame_count}.jpg")
                cv2.imwrite(temp_original_path, frame)
                
                # 2.2 调用 draw_result（按函数定义传参，无返回值）
                # 参数顺序：img_path, result_dict, data_names, is_coco_dataset, save_path
                draw_result(
                    img_path=temp_original_path,
                    result_dict=result_dict,
                    data_names=args.data.names,
                    is_coco_dataset=is_coco_dataset,
                    save_path=frame_save_dir  # 绘制后的帧保存到临时目录
                )
                
                # 2.3 读取 draw_result 保存的绘制后帧
                drawn_frame_path = os.path.join(frame_save_dir, f"original_frame_{frame_count}.jpg")  # 与原始帧同名（函数会覆盖）
                frame_with_result = cv2.imread(drawn_frame_path)
                
                # 校验绘制后帧的有效性
                if frame_with_result is None or frame_with_result.size == 0:
                    logger.warning(f"Failed to read drawn frame {frame_count}, using original frame")
                    frame_with_result = frame

                # 3. 保存视频或显示窗口
                if args.save_result and video_writer is not None:
                    video_writer.write(frame_with_result)

                if args.show_window:
                    cv2.imshow("MindYOLO Result", frame_with_result)
            ...
            logger.info(f"Video processing completed. Results saved to {args.save_dir}")
    ...
    # 图像处理逻辑
        ...
        if isinstance(args.image_path, str) and os.path.isfile(args.image_path):
            ...
            if img is None or img.size == 0:
                raise ValueError(f"Failed to read image file: {args.image_path}")
        else:
        ...
        if args.task == "detect":
            ...
            if args.save_result:
                ...
                draw_result(img=img, result=result_dict, class_names=args.data.names, 
                           is_coco_dataset=is_coco_dataset, save_path=save_path)
